        ...
        <React.Fragment>
          ...
          <div className="d-flex justify-content-center mt-5">
            {this.props.data && this.props.data.map((data,index) => {
                return (
                    <form key={index} className={`${this.state.activeStep === data.id ? '' : 'd-none'}`} style={{width:'500px'}}>
                      <SignupText className="mb-4" isOk={true}>{data.question_text}</SignupText>

                      {data.allow_many ? 
                        <CheckboxExample selectedData={(data) => { this.stepperReturn(data)}} list={data.choices}></CheckboxExample>
                        :
                        <RadioButtonExample  selectedData={(data) => { this.stepperReturn(data)}} list={data.choices} name={data.question_text}></RadioButtonExample>
                      }

                    {this.state.answersList.length > 0 ?

                        <SignupButton type="button"  className={"w-100 mt-4"} onClick={this.state.activeStep > steps.length ? null : this.checkValidations} >
                          {this.state.activeStep === steps.length ? 'Finish' : 'Next'}
                        </SignupButton>
                    :
                      <SignupButton type="button" style={{opacity:0.5}}  className={"w-100 mt-4"} > {this.state.activeStep === steps.length ? 'Finish' : 'Next'}</SignupButton>
                        
                    }

                    {this.state.activeStep ===1 
                        ? 
                        <SignupButton outline={true} type="button" className={"w-100 mt-4"}  onClick={this.props.back}>Back</SignupButton>
 
                        : 
                          <SignupButton outline={true} type="button" className={"w-100 mt-4"}  onClick={this.handleOnClickBack}>Back</SignupButton>
                        } 
                    </form>     
                ) 
              })} 
