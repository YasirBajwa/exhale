...
const RadioButton = ({ type = "radio", name, checked = false, onChange }) => {
  ...
  return (
    <input className="mr-3" type={type} name={name} checked={checked} onChange={onChange} />
  );
};
...
const RadioButtonExample = ( props) => {
  ...
  const handleRadioboxChange = (data) => {
    ...
    if (isChecked) {
      setCheckedItems(
        checkedItems.filter(
          (checkedRadio) => checkedRadio.id !== data.id
        )
      );
    } else {
  ...
  return (
    <div>
      {dataList.map((item,index) => (
        <div className="d-flex align-items-baseline mb-3 w-100" key={index}>
          <span>
            <RadioButton
              name={props.name}
